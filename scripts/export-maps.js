...
async function main() {
  ...
  await Runtime.evaluate({
    expression: `
      if (!window._xp) {
        window._xp = new PIXI.Application({width:32,height:32,preserveDrawingBuffer:true,backgroundColor:0});
        window._xp.destroy = function(){}; // 禁用销毁
      }
    `,
    returnByValue: false
  });
          ...
          (async function(){
            try {
              ...
              function doStrip(idx) {
                ...
                app.renderer.resize(tw, sh);
                app.view.width = tw; app.view.height = sh;
              ...
              if (strips <= 1) return doStrip(0);
              var res = [];
              for (var s = 0; s < strips; s++) res.push(doStrip(s));
              return JSON.stringify({s: res, sh: ${MAX_H}, fh: th});
